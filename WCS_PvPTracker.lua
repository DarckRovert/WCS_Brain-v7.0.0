--[[
    ...
    - Reemplazado date() por FormatTime() con GetTime() (WoW 1.12)
...
-- Helper: timestamp sin date() (no existe en WoW 1.12 Lua)
...
function WCS_PvPTracker:Initialize()
    ...
    -- Panel de estadísticas generales — relativo al padre
    local statsBg = CreateFrame("Frame", nil, panel)
    statsBg:SetPoint("TOPLEFT", 10, -40)
    statsBg:SetPoint("TOPRIGHT", -10, -40)   -- sin SetWidth hardcoded
    ...
    self.deathsText:SetPoint("TOP", 0, -30)
    ...
    self.duelWinsText:SetPoint("TOP", 0, -60)
    ...
    -- Panel de enemigos (columna izquierda)
    local enemiesBg = CreateFrame("Frame", nil, panel)
    enemiesBg:SetPoint("TOPLEFT", 10, -170)
    enemiesBg:SetPoint("BOTTOM", panel, "BOTTOM", 0, 10)
    enemiesBg:SetWidth(math.floor(((panel:GetWidth() or 660) - 30) / 2))
    ...
    -- Panel de kills recientes (columna derecha)
    local killsBg = CreateFrame("Frame", nil, panel)
    killsBg:SetPoint("TOPLEFT", enemiesBg, "TOPRIGHT", 10, 0)
    killsBg:SetPoint("BOTTOMRIGHT", -10, 10)
...
function WCS_PvPTracker:OnKill()
    ...
    if target then
        table.insert(pvpData.recentKills, 1, {
            name  = target,
            time  = FormatTime(),   -- GetTime()-based, sin date()
...
function WCS_PvPTracker:AddEnemy(name, class)
    for i = 1, table.getn(pvpData.enemies) do
        if pvpData.enemies[i].name == name then
            ...
            pvpData.enemies[i].lastSeen   = FormatTime()
            return
        end
    end
...
function WCS_PvPTracker:UpdateEnemies()
    ...
    -- Bubble sort por encounters (Lua 5.0 compatible)
    for i = 1, table.getn(pvpData.enemies) do
    ...
    for i = 1, table.getn(pvpData.enemies) do
        ...
        -- RAID_CLASS_COLORS usa claves en inglés con mayúsculas, e.g. "WARLOCK"
        local classKey = string.upper(enemy.class or "")
        local classColor = (RAID_CLASS_COLORS and RAID_CLASS_COLORS[classKey]) or {r=1, g=1, b=1}
        btn.text:SetText(string.format("|cff%02x%02x%02x%s|r - %s enc. - %s",
            math.floor(classColor.r * 255),
            math.floor(classColor.g * 255),
            math.floor(classColor.b * 255),
            enemy.name, enemy.encounters, enemy.lastSeen))
...
function WCS_PvPTracker:UpdateKills()
    ...
    for i = 1, math.min(20, table.getn(pvpData.recentKills)) do
        ...
        local classKey = string.upper(kill.class or "")
        local classColor = (RAID_CLASS_COLORS and RAID_CLASS_COLORS[classKey]) or {r=1, g=1, b=1}
